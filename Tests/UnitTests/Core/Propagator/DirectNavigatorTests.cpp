...
void runTest(const rpropagator_t& rprop, const dpropagator_t& dprop, double pT,
  ...
  CurvilinearTrackParameters start(Vector3(0, 0, 0), phi, theta, dcharge / p,
...
void runSimpleTest(const std::vector<const Surface*>& surfaces,
  ...
  BoundTrackParameters startParameters =
      BoundTrackParameters(startSurface->getSharedPtr(),
                           {0.0_mm, 0.0_mm, 0.0_rad, 0.0_rad, 1.0 / 1.0_GeV},
                           std::nullopt, ParticleHypothesis::muon());
