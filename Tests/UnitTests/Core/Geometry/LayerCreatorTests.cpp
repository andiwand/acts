...
struct LayerCreatorFixture {
  ...
  /// Every bin has to hold at least @p n surfaces. A surface is registered in
  /// every bin its projection overlaps, so a bin sitting on a module boundary
  /// legitimately holds more than the modules whose centre it contains.
