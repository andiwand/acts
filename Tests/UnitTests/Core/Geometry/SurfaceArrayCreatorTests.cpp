...
// The point of the phi rotation is that module centres end up in the middle of
// their bin, whatever phi the detector happens to start at. Checking the
// rotation angle alone cannot tell the intended sign from its opposite: both
// agree when the detector starts at a multiple of half a bin, and the opposite
// sign doubles the offset everywhere else.
BOOST_FIXTURE_TEST_CASE(SurfaceArrayCreator_phiAxisAlignsModuleCentres,
  ...
  for (std::size_t i = 0; i <= 8; ++i) {
    ...
    // `tr` is the local-to-global transform of the representative surface
    ...
    for (const Surface* surface : surfacesRaw) {
      ...
      // 0.5 is the bin centre; the inverted sign lands on 0.0 / 1.0 for
      // shift = step / 4
...
BOOST_FIXTURE_TEST_CASE(SurfaceArrayCreator_neighborWindowFollowsCrossingAngle,
  ...
  const auto missedHits = [&](std::array<std::uint8_t, 2> maxNeighborDistance) {
  ...
  BOOST_CHECK_EQUAL(missedHits({1, 2}), 0u);
  // the fixed one-bin window this replaces cannot reach that far
  BOOST_CHECK_GT(missedHits({1, 1}), 0u);
}
...
// The axis values are derived from the surfaces, but they are looked up as
// local coordinates of the representative surface. `LayerCreator` places that
// surface at the layer centre, so for a barrel that is not centred at z = 0 the
// two frames differ by the layer z position.
BOOST_FIXTURE_TEST_CASE(SurfaceArrayCreator_zAxisInRepresentativeFrame,
  ...
  // the z axis has to be centred on the representative surface, not on z = 0
  CHECK_CLOSE_ABS(0.5 * (axes.at(1)->getMin() + axes.at(1)->getMax()), 0.,
                  1e-6);

  // every module has to land in its own z bin rather than being clamped into
  // the edge bin, which is what an axis in the global frame would do
  std::set<std::size_t> occupiedZBins;
  for (const Surface* surface : surfacesRaw) {
    const Vector3 centre = surface->center(tgContext);
    // radial direction, which is the representative cylinder's normal
...
BOOST_FIXTURE_TEST_CASE(SurfaceArrayCreator_barrelStagger,
  ...
  // the axes are expressed in the frame of the representative surface, so it
  // has to be built with the transform the axis creation handed back
