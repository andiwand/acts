...
void addFatrasSynthetic(py::module_& fatras) {
  ...
  py::enum_<EndcapPlacement>(m, "EndcapPlacement")
      .value("Negative", EndcapPlacement::Negative)
      .value("Positive", EndcapPlacement::Positive)
      .value("Mirrored", EndcapPlacement::Mirrored);
  ...
  py::class_<DetectorLayout>(m, "DetectorLayout")
      ...
      .def_readonly("subsystems", &DetectorLayout::subsystems)
  ...
  py::class_<DetectorLayoutBuilder>(m, "DetectorLayoutBuilder")
      .def(py::init<std::string>(), py::arg("subsystem") = std::string{})
      .def("beginSubsystem", &DetectorLayoutBuilder::beginSubsystem,
      ...
      .def(
          ...
          [](DetectorLayoutBuilder& builder, SurfaceSide side, float absZ,
             const std::vector<RingBounds>& rings,
             std::optional<std::uint32_t> layer) -> DetectorLayoutBuilder& {
            return builder.addDisc(side, absZ, rings, layer);
          },
      ...
      .def("addDisc",
           ...
           py::arg("numRings"), py::arg("layer") = std::nullopt,
           py::return_value_policy::reference_internal)
  ...
  py::class_<CylinderDescription>(m, "CylinderDescription")
      .def(py::init<>())
      .def_readwrite("radius", &CylinderDescription::radius)
      .def_readwrite("halfLengthZ", &CylinderDescription::halfLengthZ)
      .def_readwrite("modules", &CylinderDescription::modules)
      .def_readwrite("overlapProbability",
  ...
  py::class_<PassiveSurfaceDescription>(m, "PassiveSurfaceDescription")
      ...
      .def_readwrite("placement", &PassiveSurfaceDescription::placement)
      .def_readwrite("minBound", &PassiveSurfaceDescription::minBound)
      .def_readwrite("maxBound", &PassiveSurfaceDescription::maxBound)
      .def_readwrite("layer", &PassiveSurfaceDescription::layer)
      .def_readwrite("material", &PassiveSurfaceDescription::material);

  py::class_<BarrelDescription>(m, "BarrelDescription")
      .def(py::init<>())
      .def_readwrite("cylinders", &BarrelDescription::cylinders);

  py::class_<EndcapDescription>(m, "EndcapDescription")
      .def(py::init<>())
      .def_readwrite("placement", &EndcapDescription::placement)
      .def_readwrite("discs", &EndcapDescription::discs);

  py::class_<SubsystemDescription>(m, "SubsystemDescription")
      .def(py::init<>())
      .def_readwrite("name", &SubsystemDescription::name)
      .def_readwrite("barrels", &SubsystemDescription::barrels)
      .def_readwrite("endcaps", &SubsystemDescription::endcaps)
      .def_readwrite("passives", &SubsystemDescription::passives);
  ...
  py::class_<DetectorDescription>(m, "DetectorDescription")
      ...
      .def_readwrite("escapeHalfZ", &DetectorDescription::escapeHalfZ);
  ...
  m.def("assignLayerIndices", &assignLayerIndices, py::arg("description"));
  m.def(
      "selectSubsystems",
      [](const DetectorDescription& description,
         const std::vector<std::string>& names) {
        return selectSubsystems(description, names);
      },
      py::arg("description"), py::arg("names"));
  m.def(
      "merge",
      [](const std::vector<DetectorDescription>& descriptions) {
        return merge(descriptions);
      },
      py::arg("descriptions"));
  ...
  py::enum_<LayerKind>(m, "LayerKind")
      .value("Barrel", LayerKind::Barrel)
      .value("Endcap", LayerKind::Endcap)
      .value("Passive", LayerKind::Passive);

  py::class_<LayerId>(m, "LayerId")
      .def(py::init<>())
      .def_readwrite("subsystem", &LayerId::subsystem)
      .def_readwrite("kind", &LayerId::kind)
      .def_readwrite("placement", &LayerId::placement)
      .def_readwrite("layer", &LayerId::layer);

  py::class_<MaterialEntry>(m, "MaterialEntry")
      .def(py::init<>())
  ...
  // the JSON plugin; see `addFatrasSyntheticJson`.
  addFatrasSyntheticJson(m);
#endif
  ...
  py::class_<TrackingGeometryLayoutOptions>(m, "TrackingGeometryLayoutOptions")
      ...
      .def(
          "setSubsystemName",
          [](TrackingGeometryLayoutOptions& options,
             std::function<std::string(const Acts::Surface*)> name) {
            options.subsystemName =
                [name = std::move(name)](const Acts::Surface& surface) {
                  return name(&surface);
                };
          },
          py::arg("name"))
      .def_readwrite("escapeRadius",
                     &TrackingGeometryLayoutOptions::escapeRadius)
      .def_readwrite("escapeHalfZ", &TrackingGeometryLayoutOptions::escapeHalfZ)
  ...
  m.def("makeDescriptionFromTrackingGeometry",
        &makeDescriptionFromTrackingGeometry, py::arg("trackingGeometry"),
        py::arg("gctx"), py::arg("options") = TrackingGeometryLayoutOptions{});
