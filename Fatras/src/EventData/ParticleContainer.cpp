...
ParticleContainer &ParticleContainer::operator=(
    const ParticleContainer &other) noexcept {
  if (this == &other) {
    return *this;
  }
...
ParticleContainer &ParticleContainer::operator=(
    ParticleContainer &&other) noexcept {
  if (this == &other) {
    return *this;
  }
...
void ParticleContainer::moveColumns(ParticleContainer &other) noexcept {
  m_allColumns.reserve(other.m_allColumns.size());
  m_dynamicColumns.reserve(other.m_dynamicColumns.size());
  ...
  other.m_knownColumns = ParticleColumns::None;
...
MutableParticleProxy ParticleContainer::createParticle() noexcept {
  ++m_size;
...
void ParticleContainer::createColumns(ParticleColumns columns) noexcept {
  using enum ParticleColumns;

  const auto createColumn =
      [&]<typename T>(ParticleColumns mask, std::string_view name,
...
void ParticleContainer::dropColumns(ParticleColumns columns) noexcept {
  using enum ParticleColumns;

  const auto dropColumn = [&]<typename T>(
                              ParticleColumns mask, std::string_view name,
                              std::optional<ColumnHolder<T>> &column) {
    if (ACTS_CHECK_BIT(columns, mask) && column.has_value()) {
...
void ParticleContainer::dropColumn(const std::string &name) {
...
bool ParticleContainer::reservedColumn(const std::string &name) noexcept {
