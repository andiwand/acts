...
inline PositionedIntersection<3> intersectPlane(const Vector3& linePos,
  ...
  if (std::abs(normDot) < std::numeric_limits<double>::epsilon()) {
    return PositionedIntersection<3>::Invalid();
  }
  const double path = (offset - linePos.dot(planeNorm)) / normDot;
  return PositionedIntersection<3>{linePos + path * lineDir, path,
