...
Acts::Result<Acts::LinearizedTrack> Acts::
    HelicalTrackLinearizerWithTime<propagator_t, propagator_options_t>::linearizeTrack(
  ...
  //parameters of the original track at the linearization point (i.e., the PCA to the vertex, i.e., the point V)
  BoundVector paramsAtPCA = endParams.parameters();
  //this needs to be included in the linearization point in the future
  double ref_time_point = 0.; //I don't think we need this here since R = linPointPos

  //time needs to be relative to the new perigee (just like its spatial parameters)
  ...
  */
  ...
  if (parCovarianceAtPCA.determinant() <= 0) {
    ...
    auto pos = endParams.position(gctx);
    //the following 3 lines are the same in the if confition above - shouldn't they be outside the if then?
    positionAtPCA[ePos0] = pos[ePos0];
    positionAtPCA[ePos1] = pos[ePos1];
    positionAtPCA[ePos2] = pos[ePos2];
    //Add timing
...
/*
  ...
  */
  ...
  //if the charge is negative the particle moves counterclockwise (when looking in negative z-direction)
  //therefore the sign of the curvature is negative
  ...
  predParamsAtPCA[1] =
      positionAtPCA[eZ] - linPointPos.z() + rho * (phiV - phiAtPCA) / tanTh; //sign mistake in Ref(1) - here the equation is correct
  ...
  ActsScalar massInGeV = 0.1057; //in GeV - pion, mass hypothesis! TODO: find out why
  ...
  //I think we should write paramsAtPCA[eBoundTime] instead of positionAtPCA[eTime] - linPoint[eTime]
  predParamsAtPCA[5] = positionAtPCA[eTime] - linPoint[eTime] + extrap_term; //should be relative to linPoint
...
/*
  ...
  */
  ...
  momentumJacobian(5, 2) = oneOverOmega/qOvP*(beta*beta*dPhi-R*rhoOverS2);
  ...
  BoundVector constTerm = predParamsAtPCA - positionJacobian * positionAtPCA -
                          momentumJacobian * momentumAtPCA;//should it not be paramsAtPCA instead of predParamsAtPCA?
...
template <typename propagator_t, typename propagator_options_t>
void Acts::
    HelicalTrackLinearizerWithTime<propagator_t, propagator_options_t>::calculateNumericalJacobians(
        const BoundTrackParameters& params, const Vector4& linPoint,
        const Acts::GeometryContext& gctx,
        const Acts::MagneticFieldContext& mctx, State& state) const {
  //estimated vertex position
  ...
  Vector3 posShifted;
  bool shiftRef = false;
  BoundVector paramsAtPCAShifted;
  ActsMatrix<eBoundSize, 4> positionJacobian;
  positionJacobian.setZero();
  ActsMatrix<eBoundSize, 3> momentumJacobian;
  momentumJacobian.setZero();
  ...
  std::cout << "\nendParams:\n" <<endParams <<"\n";
  for (int i=0; i<3; i++){
  SingleBoundTrackParameters endParamsCopy = endParams;
  if (shiftRef){
    ...
    globalCoords(i) += delta;
    //std::cout << "\nglobal shifted\n" << globalCoords << "\n" << "\nglobal\n" << endParams.position(gctx) << "\n";
    auto shiftedD0Z0 = *endParamsCopy.referenceSurface().globalToLocal(gctx, globalCoords, endParams.momentum());
    
    //std::cout << "\nshiftedd0z0\n" << shiftedD0Z0 << "\n" << "\nd0z0\n" << endParamsCopy.parameters().head(2) << "\n";
    endParamsCopy.parameters().template head<2>() = shiftedD0Z0;
  }
  std::cout << "\nendParams after wiggle:\n" <<endParamsCopy <<"\n";
  intersection = perigeeSurface->intersect(gctx, globalCoords,
                                          endParamsCopy.unitDirection(), false);
  // Create propagator options
  pOptions.direction = intersection.intersection.pathLength >= 0
                          ? NavigationDirection::Forward
                          : NavigationDirection::Backward;
                        
  if (!shiftRef) globalCoords(i) -= delta;
  // Do the propagation to the PCA of linPointPos
  result = m_cfg.propagator->propagate(endParamsCopy, *perigeeSurface, pOptions);

  //parameters of the original track at the linearization point (i.e., the PCA to the vertex, i.e., the point V)
  auto shiftedResult = (*result->endParameters);
  paramsAtPCAShifted = shiftedResult.parameters();
  std::cout << "\nendParams after shift:\n" <<paramsAtPCAShifted <<"\n";
  //std::cout << "\nshifted params:\n" << paramsAtPCAShifted << "\nparams:\n" << paramsAtPCA << "\n";
  std::cout << "\ncolumn no. " << i << "\n"; 
  for(int j=0; j<6; j++){
    //positionJacobian(i, j) = (paramsAtPCAShifted[j]-paramsAtPCA[j])/delta;
    std::cout << j <<": " << (paramsAtPCAShifted[j]-paramsAtPCA[j])/delta << "\n";
    }
  } 
}
