...
Acts::Result<Acts::LinearizedTrack> Acts::
  ...
  Vector3 linPointPos = VectorHelpers::position(linPoint); //this corresponds to R
  ...
  const auto& endParams = *result->endParameters; //these are the Perigee params of V
  ...
  double X = positionAtPCA(0) - linPointPos.x() + rho * sinPhiV; //shouldn't it be sin(phiV+pi/2)
...
template <typename propagator_t, typename propagator_options_t>
void Acts::
    HelicalTrackLinearizer<propagator_t, propagator_options_t>::testLinearization(
        const BoundTrackParameters& params, const Vector4& linPoint,
        const Acts::GeometryContext& gctx,
        const Acts::MagneticFieldContext& mctx, State& state) const {
          Vector3 linPointPos = VectorHelpers::position(linPoint); //this corresponds to R
          ...
          /// (see FullBilloirVertexFitter.hpp for paper reference,
          /// Page 140, Eq. (2) )
          BoundVector predParamsAtPCA; //this are the Perigee params at point P
          ...
          predParamsAtPCA[0] = rho - sgnH * S;
          predParamsAtPCA[1] =
              positionAtPCA[eZ] - linPointPos.z() + rho * (phiV - phiAtPCA) / tanTh;//positionAtPCA[eZ] = z_v, linPointPos.z() = z_r
          predParamsAtPCA[2] = phiAtPCA;
          predParamsAtPCA[3] = th;
          predParamsAtPCA[4] = qOvP;
          predParamsAtPCA[5] = 0.;
          ...
          BoundVector qLin = positionJacobian* + momentumJacobian* 
