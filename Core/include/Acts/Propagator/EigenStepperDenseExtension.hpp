...
void updateAdditionalFreeCovariance(const propagator_state_t& state,
  ...
  // handle multiple scattering
  {
    ...
    float theta0 = computeMultipleScatteringTheta0(accumulatedMaterial, absPdg,
                                                   mass, *firstQOverP, absQ);
  ...
  {
    float qOverPSigma = computeEnergyLossLandauSigmaQOverP(accumulatedMaterial,
                                                           mass, qOverP, absQ);
