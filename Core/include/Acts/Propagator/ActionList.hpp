...
struct ActionList : public detail::Extendable<actors_t...> {
 ...
 public:
  ...
  template <typename result_t, typename propagator_state_t, typename stepper_t, typename navigator_t,
            typename... Args>
  void operator()(result_t& result, propagator_state_t& state,
                  const stepper_t& stepper, const navigator_t& navigator, Args&&... args) const {
    using impl = detail::action_list_impl<actors_t...>;
    impl::action(tuple(), result, state, stepper, navigator, std::forward<Args>(args)...);
  }
};
