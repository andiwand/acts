...
struct GbtsEtaBinInfo final {
  ...
  /// Whether the layer this bin belongs to is a pixel layer, which is constant
  /// over a bin and so hoists out of the pair loops. A strip node carries a
  /// stereo pair the graph has to resolve before it can cut on the pair's
  /// direction; see `GbtsNodeStorage::strip`.
  bool isPixel{true};
