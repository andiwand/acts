...
CylindricalSpacePointGrid CylindricalSpacePointGridCreator::createGrid(
  ...
  return CylindricalSpacePointGrid(
      std::make_tuple(std::move(phiAxis), std::move(zAxis), std::move(rAxis)));
}

template <typename spacepoint_iterator_t>
void CylindricalSpacePointGridCreator::fillGrid(
    const SeedFinderConfig& config, CylindricalSpacePointGrid& grid,
    const SpacePointContainer& collection, spacepoint_iterator_t spBegin,
    spacepoint_iterator_t spEnd, const Logger& logger) {
  ...
  for (std::size_t binIndex : rBinsIndex) {
    ...
    std::ranges::sort(
        rbin, {}, [&](const auto& rb) { return collection.at(rb).radius(); });
  }
...
void CylindricalSpacePointGridCreator::fillGrid(
    const SeedFinderConfig& config, CylindricalSpacePointGrid& grid,
    const SpacePointContainer& collection, const Logger& logger) {
  CylindricalSpacePointGridCreator::fillGrid(
      config, grid, collection, collection.begin(), collection.end(), logger);
}
