...
class GbtsNodeStorage final {
 public:
  ...
  /// @param clusterWidth Pixel cluster width
  /// @param localPositionY Local y cluster position
  /// @param strip Stereo pair the point was formed from, for a strip space
  ///        point; null for a pixel one. Kept so that the graph can put the
  ///        point back where the direction of a doublet says it is.
  /// @return The eta bin the node was placed in, or nullopt if it was rejected
 ...
 private:
  ...
  ///
  /// @pre The node is one, i.e. its eta bin is not a pixel bin. There is no
  ///      entry for a pixel node, the side array holding only what the strip
  ///      layers put in it.
  ///
  /// @param node Node index
  /// @return The pair
  ...
  /// Whether any node carries a stereo pair at all, which is what decides
  /// whether the graph has a strip path to take.
  ...
  /// Stereo pairs of the strip nodes, in node order and compacted: 60 bytes
  /// each is too much to carry for every node of a mostly pixel detector.
