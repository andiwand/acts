...
inline LinCircle transformCoordinates(SpacePointMutableData& mutableData,
                                      ConstSpacePointProxy sp,
                                      ConstSpacePointProxy spM, bool bottom,
...
inline void transformCoordinates(const SpacePointContainer& spacePointContainer,
  ...
  for (std::size_t idx(0); idx < vec.size(); ++idx) {
    auto sp = spacePointContainer.at(idx);
...
inline bool xyzCoordinateCheck(const SeedFinderConfig& m_config,
                               ConstSpacePointProxy sp,
