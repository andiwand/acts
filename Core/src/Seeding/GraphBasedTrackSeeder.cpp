...
std::pair<std::uint32_t, std::uint32_t> GraphBasedTrackSeeder::buildTheGraph(
  ...
  for (const auto& bg : m_geometry->binGroups()) {
    ...
    // Inside-out ordinal within the pixel barrel, -1 elsewhere. The adaptive
    // tau corrections and the triplet validation below were tuned on the pixel
    // barrel and are keyed on it: a non-negative ordinal is exactly what
    // ATLAS's (layerId / 10000) == 8 selects, its strip barrel being 13xxx.
