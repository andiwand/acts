...
struct BilloirTrack {
  ...
  Acts::ActsMatrix<Acts::eBoundSize, 3> D;  // Di (position Jacobian)
  Acts::ActsMatrix<Acts::eBoundSize, 3> E;  // Ei (momentum Jacobian)
  Acts::SquareMatrix3 C;                    //  = sum{Ei^T Wi * Ei}
  Acts::ActsMatrix<3, 3> B;                 //  = Di^T * Wi * Ei
  Acts::SquareMatrix3 Cinv;                 //  = (Ei^T * Wi * Ei)^-1
  Acts::Vector3 U;                          //  = Ei^T * Wi * dqi
  Acts::SquareMatrix3 BCinv;                //  = Bi * Ci^-1
...
struct BilloirVertex {
  ...
  // T  = sum{Di^T * Wi * dqi}
  Acts::Vector3 T = Acts::Vector3::Zero();
  // sumBCinvBt = sum{Bi * Ci^-1 * Bi^T}
  Acts::SquareMatrix3 sumBCinvBt = Acts::SquareMatrix3::Zero();
  // sumBCinvU = sum{B * Ci^-1 * Ui}
  Acts::Vector3 sumBCinvU = Acts::Vector3::Zero();
};
...
Acts::Result<Acts::Vertex> Acts::FullBilloirVertexFitter::fit(
  ...
  Vector3 linPoint = vertexingOptions.constraint.position();
  Vertex fittedVertex;
  ...
  for (int nIter = 0; nIter < m_cfg.maxIterations; ++nIter) {
    ...
    for (std::size_t iTrack = 0; iTrack < nTracks; ++iTrack) {
      ...
      auto result = m_cfg.trackLinearizer(
          trackParams, *perigeeSurface, vertexingOptions.geoContext,
      ...
      ActsMatrix<eBoundSize, 3> D = linTrack.positionJacobian;
      ...
      ActsMatrix<3, eBoundSize> DtW = D.transpose() * W;
    ...
    if (vertexingOptions.useConstraintInFit) {
      ...
      Vector3 posInBilloirFrame =
          vertexingOptions.constraint.position() - linPoint;
      ...
      deltaVFac += vertexingOptions.constraint.covariance().inverse() *
                   posInBilloirFrame;
      // For vertex constraint: A -> A + Cb^-1
      invCovV += vertexingOptions.constraint.covariance().inverse();
    }

    // Covariance matrix of the 4D vertex position, see Ref. (3)
    SquareMatrix3 covV = invCovV.inverse();
    // Update of the vertex position
    Vector3 deltaV = covV * deltaVFac;
    //--------------------------------------------------------------------------------------
    ...
    for (std::size_t iTrack = 0; iTrack < billoirTracks.size(); ++iTrack) {
      ...
      // d(d0,z0,phi,theta,qOverP)/d(x,y,z,phi,theta,qOverP)
      ...
      ActsMatrix<eBoundSize, eLinSize> transMat;
      transMat.setZero();
      ...
      // cov(V,P)
      ActsMatrix<3, 3> covVP = -covV * billoirTrack.BCinv;

      // cov(P,P), 3x3 matrix, see Ref. (3)
      SquareMatrix3 covP = billoirTrack.Cinv + billoirTrack.BCinv.transpose() *
                                                   covV * billoirTrack.BCinv;

      ActsSquareMatrix<eLinSize> cov;
      cov.setZero();
      cov.block<3, 3>(0, 0) = covV;
      cov.block<3, 3>(0, 3) = covVP;
      cov.block<3, 3>(3, 0) = covVP.transpose();
      cov.block<3, 3>(3, 3) = covP;
    ...
    if (vertexingOptions.useConstraintInFit) {
      ...
      newChi2 += (posInBilloirFrame.transpose())
                     .dot(vertexingOptions.constraint.covariance().inverse() *
                          posInBilloirFrame);
    }
    ...
    if (newChi2 < chi2) {
      ...
      fittedVertex.setCovariance(covV);
