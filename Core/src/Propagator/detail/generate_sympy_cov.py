...
# The bound-to-bound jacobian's shape, as generate_sympy_jac.py produces it
# and asserts: nothing depends on time, and q/p depends on nothing but itself.
#
# Its q/p diagonal is one only in vacuum, where the step kernel never writes
# that entry (its live set stops at row 6) so it keeps the value initialisation
# gave it. A dense step does write it -- energy loss makes d(q/p out)/d(q/p in)
# differ from one -- so the two cases get their own transport: the vacuum one
# folds the diagonal away as a literal, the dense one carries it.
