...
Result<double> sympyDenseStepFull(const SympyStepper& stepper,
  ...
  while (true) {
    ...
    if (!state.options.doDense || material == nullptr) {
      ...
      Result<bool> res =
          sympyDenseStep(stepper, state, *material, timeDirection, h,
                         4 * stepTolerance, errorEstimate, lastField, jac);
      if (!res.ok()) {
        return res.error();
      }
      accepted = *res;
...
Result<bool> sympyDenseStep(const SympyStepper& stepper,
  ...
  const auto getG = [&](std::span<const double, 3> p, double l) -> double {
    const double newPabs = stepper.particleHypothesis(state).extractMomentum(l);
    ...
    if (state.options.dense.meanEnergyLoss) {
      return timeDirection * computeEnergyLossMean(slab, absPdg, m, l, absQ);
    }
    return timeDirection * computeEnergyLossMode(slab, absPdg, m, l, absQ);
  };
