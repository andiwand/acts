...
struct SurfaceArray::ISurfaceGridLookup {
  ...
  /// Get the upper bound on the neighbor window this lookup can serve, per
  /// axis. The window itself is derived per axis from the crossing angle and
  /// is capped at these values.
  /// @return Maximum neighbor distance per axis
  virtual std::array<std::uint8_t, 2> maxNeighborDistance() const = 0;
};
...
struct SingleElementLookupImpl final : SurfaceArray::ISurfaceGridLookup {
  ...
  std::array<std::uint8_t, 2> maxNeighborDistance() const override {
    return {0, 0};
  }
...
struct SurfaceGridLookupImpl final : SurfaceArray::ISurfaceGridLookup {
  SurfaceGridLookupImpl(std::shared_ptr<RegularSurface> representative,
                        ...
                        std::array<std::uint8_t, 2> maxNeighborDistance = {2,
                                                                           2})
 ...
 private:
  ...
  GridDistance m_maxNeighborDistance{};
  ...
  std::size_t neighborDistanceStride(std::size_t axis) const {
    return static_cast<std::size_t>(m_maxNeighborDistance.at(axis)) + 1;
  }
  ...
  /// one only multiplies the candidate count.
  GridDistance crossingNeighborDistance(const GeometryContext& gctx,
    ...
    const GridDistance maximum = m_maxNeighborDistance;
    ...
    return {std::min(neighborDistance[0], m_maxNeighborDistance[0]),
            std::min(neighborDistance[1], m_maxNeighborDistance[1])};
  }
  ...
  void populateNeighborCache() {
    ...
    for (std::size_t inputGlobalBin = 0; inputGlobalBin < m_fillingGrid.size();
      ...
      for (std::uint8_t distance0 = 0; distance0 <= m_maxNeighborDistance[0];
           ++distance0) {
        for (std::uint8_t distance1 = 0; distance1 <= m_maxNeighborDistance[1];
             ++distance1) {
          surfacePack.clear();
...
std::unique_ptr<SurfaceArray::ISurfaceGridLookup> makeSurfaceGridLookup(
    ...
    std::array<std::uint8_t, 2> maxNeighborDistance) {
...
std::array<std::uint8_t, 2> SurfaceArray::maxNeighborDistance() const {
