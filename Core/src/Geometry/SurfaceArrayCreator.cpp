...
/// Offset between the frame the axis values are measured in and the frame they
/// are looked up in.
///
/// The bin edges are derived from the surfaces, i.e. they are expressed in the
/// frame the @c ProtoLayer was measured in, but they are consumed as local
/// coordinates of the representative surface. Subtracting the representative
/// surface's translation puts them in the same frame.
///
/// Only the translation is handled here. The phi rotation is applied by
/// @c createEquidistantAxis itself, and R is invariant under the translations
/// the surface array creation applies (which are always along the
/// representative surface's own axis).
...
std::unique_ptr<const IAxis> SurfaceArrayCreator::createEquidistantAxis(
  ...
  if (aDir == AxisPhi) {
    ...
    if (m_cfg.doPhiBinningOptimization) {
      ...
      // Rotate so that the highest-phi module centre lands on the centre of
      // the last bin, i.e. so that the wrap-around at +-pi never falls on a
      // module centre.
      //
      // @p transform is the local-to-global transform of the representative
      // surface, so the lookup sees `localPhi = globalPhi - angle`. Solving
      // `gridMax = surfaceMax - angle` therefore gives the rotation below; the
      // opposite sign doubles the module offset instead of cancelling it.
