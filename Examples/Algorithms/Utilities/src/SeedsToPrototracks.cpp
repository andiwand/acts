...
ProcessCode SeedsToPrototracks::execute(const AlgorithmContext& ctx) const {
  ...
  std::transform(seeds.begin(), seeds.end(), std::back_inserter(tracks),
                 [&](const auto& seed) {
                   return seedToPrototrack(seed, spacePointContainer);
                 });
