...
Pythia8Generator::operator()(RandomEngine& rng) {
  ...
  vertices.emplace_back(0, SimVertex::Vector3(0., 0., 0.));
  ...
  for (int ip = 0; ip < m_pythia8->event.size(); ++ip) {
    ...
    SimParticle::Vector3 pos(genParticle.xProd() * 1_mm,
    ...
    if (m_cfg.labelSecondaries && genParticle.hasVertex()) {
      ...
      if (it != vertices.end()) {
        ...
        auto& vertex = vertices.emplace_back(particleId.vertexId(), pos);
