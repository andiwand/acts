...
CsvProtoTrackWriter::CsvProtoTrackWriter(
    const CsvProtoTrackWriter::Config& config, Acts::Logging::Level level)
...
ProcessCode CsvProtoTrackWriter::writeT(const AlgorithmContext& ctx,
  ...
  for (auto trackId = 0ul; trackId < tracks.size(); ++trackId) {
    for (Index measurmentId : tracks[trackId]) {
      const auto spr =
          findSpacePointByMeasurementIndex(measurmentId, spacepoints);
      if (!spr.has_value()) {
