...
ProcessCode CsvSimHitReader::read(const AlgorithmContext& ctx) {
  ...
  while (reader.read(data)) {
    ...
    const Acts::GeometryIdentifier geometryId(data.geometry_id);

    const SimParticleIndex particleId(data.particle_id);
    ...
    const std::int32_t particleHitIndex = data.particle_hit_index;

    hits.emplace_back(geometryId, particleId, pos4, mom4, mom4 + delta4,
                      particleHitIndex);
  }
