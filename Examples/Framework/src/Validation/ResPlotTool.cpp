...
void ResPlotTool::fill(const Acts::GeometryContext& gctx,
  ...
  truthParameters[eBoundPhi] = phi(truthParticle.generationState().direction());
  truthParameters[eBoundTheta] =
      theta(truthParticle.generationState().direction());
  truthParameters[eBoundQOverP] = truthParticle.generationState().qOverP();
  truthParameters[eBoundTime] = truthParticle.generationState().time();

  // get the truth eta and pT
  const double truthEta = eta(truthParticle.generationState().direction());
  const double truthPhi = phi(truthParticle.generationState().direction());
  const double truthPt = truthParticle.generationState().transverseMomentum();
